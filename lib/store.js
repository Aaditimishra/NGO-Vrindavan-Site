...
/* Storage down ho jaye (jaise Blob store suspend) to bhi website chalti rehni chahiye.
   Aisi sthiti me deploy ke saath aayi data/content.json dikhate hain (sirf padhne ke liye)
   aur admin ko saaf error batate hain — poori site 500 nahi hoti. */
const health = { ok: true, error: null, at: 0 };
function markDown(e) {
  health.ok = false;
  health.error = e && e.message ? e.message : String(e);
  health.at = Date.now();
  console.error('[store] storage uplabdh nahi:', health.error);
}
function markUp() { if (!health.ok) console.log('[store] storage wapas chalu'); health.ok = true; health.error = null; }
...
async function getContent(fresh = false) {
  ...
  if (BLOB) {
    try {
      ...
      markDown(e);
      data = cache.content || seedContent();       // website chalti rahe
    }
  } else {
...
async function getConfig(fresh = false) {
  ...
  if (BLOB) {
    try {
      ...
      markDown(e);
      cfg = cache.config || defaultConfig();       // env ke ADMIN_* se login chalta rahega
      delete cfg._plainOnce;
    }
  } else {
...
async function setConfig(cfg) {
  if (BLOB) {
    try { await blobWrite(CONFIG_KEY, cfg); } catch (e) { markDown(e); throw new Error('STORAGE_DOWN'); }
  }
...
async function saveUpload(buffer, originalName, mimetype) {
  ...
  if (BLOB) {
    ...
    } catch (e) { markDown(e); throw new Error('STORAGE_DOWN'); }
  }
...
module.exports = { getContent, setContent, getConfig, setConfig, saveUpload, listUploads, isBlob: BLOB, health };
